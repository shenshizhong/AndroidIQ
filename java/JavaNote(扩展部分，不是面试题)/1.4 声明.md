...
最终会调用c++
bool compareAndSwapInt(volatile int* ptr, int expected, int newValue){
    lock bus_lock; //lock 前缀锁定内存总线，其他cpu 不能访问，整个操作再一个指令周期完成
    int currentValue = *ptr;
    if(currentValue == expected){//如果没人改过，那么就能执行，并设置设置新值
        *ptr = newValue;
        return true; //成功
    }
    return false；
