...
2、object 声明一个静态类，类中的方法和变量都是静态的
3、用于单例
4、用于制作工具类，比如ToastUtils


companion object：
被外层类静态调用，可以静态调用companion object中的方法，变量

总结:object 是为了声明静态类，加上companion 是为了实现静态内部类

```
[详细地址：https://www.jianshu.com/p/14db81e1576a](https://www.jianshu.com/p/14db81e1576a)
...
class CountingSet<T>(val innerSet: MutableCollection<T> = HashSet<T>()) :
      ...
      var objectsAdd = 0
      override fun add(element: T): Boolean{//代理中进行重写，被下面的方法调用
          objectsAdded++
          return innerSet.add(element)
      }
      override fun addAll(c: Collection<T>): Boolean{ //重写
          objectsAdded++
...
println("${cset.objectsAdded} objects were added")
3 objects were added
```
怎么使用协变
```
协变：也就是一种约定，协议
...
out 有两层意思：
子类型化会被保留（Producer<Cat> 是 Producer<Animal> 子类型）
T 只能放在 out 位置
```
