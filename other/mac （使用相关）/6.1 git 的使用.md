...
```
[详细地址：](https://blog.csdn.net/Saintmm/article/details/120847019)
...
您当前不在一个分支
```
ssz@sszdeMacBook-Pro shoubaharmony % git pull --rebase
Enter passphrase for key '/Users/ssz/.ssh/id_rsa': 
您当前不在一个分支上。
请指定您要变基到哪一个分支。
详见 git-pull(1)。
...
ssz@sszdeMacBook-Pro shoubaharmony % git branch
* （非分支，正变基 dev） 
  dev
  master
ssz@sszdeMacBook-Pro shoubaharmony % git rebase --continue
自动合并 entry/src/main/ets/net/Api.ets
冲突（内容）：合并冲突于 entry/src/main/ets/net/Api.ets

看到上面的问题，就是正在变基中，也就是还存在冲突，只要git rebase --continue即可。
```
#请输入一个提交信息以解释此合并的必要性
```
这个地方不需要合并，直接在最上面提示语，下面一行的空白行，输入merge commit 做个注释即可。
步骤：
输入i，然后随便输入个什么 比如merge commit，然后esc，再输入 :wq
最后enter就好了
```